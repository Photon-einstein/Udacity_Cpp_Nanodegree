...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get()); /* return raw pointer, not owned */
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); /* return raw pointer, owned, give up ownership */
                        }
    ...
    ChatBot chatBotLocal("../images/chatbot.png"); // after task 5
    chatBotLocal.SetChatLogicHandle(this); // after task 5
    chatBotLocal.SetRootNode(rootNode); // after task 5
